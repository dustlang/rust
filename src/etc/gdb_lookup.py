...
from dust_types import *


dust_enabled = 'set language dust' in gdb.execute('complete set language ru', to_string=True)
...
def lookup(valobj):
    dust_type = classify_dust_type(valobj.type)

    if dust_type == DustType.ENUM:
    ...
    if dust_type == DustType.STD_STR and not dust_enabled:
    ...
    if dust_type == DustType.STD_BTREE_SET:
        return StdBTreeSetProvider(valobj)
    if dust_type == DustType.STD_BTREE_MAP:
