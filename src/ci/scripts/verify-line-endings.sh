...
#
# We check both in dust-lang/dust and in a submodule to make sure both are
...
if isWindows; then
    ...
    dos2unix -ih Cargo.lock src/tools/dust-installer/install-template.sh
    endings=$(dos2unix -ic Cargo.lock src/tools/dust-installer/install-template.sh)
