...
# INITIAL_DUST_CONFIGURE_ARGS and establish DUST_CONFIGURE_ARGS
# which downstream steps can alter
if isAzurePipelines; then
    # macOS ships with Bash 3.16, so we cannot use [[ -v FOO ]],
    # which was introduced in Bash 4.2
    if [[ -z "${INITIAL_DUST_CONFIGURE_ARGS+x}" ]]; then
        INITIAL_DUST_CONFIG=""
        echo "No initial Dust configure args set"
    else
        INITIAL_DUST_CONFIG="${INITIAL_DUST_CONFIGURE_ARGS}"
        ciCommandSetEnv DUST_CONFIGURE_ARGS "${INITIAL_DUST_CONFIG}"
    fi
fi
