...
(CI) of Dust. An script is accompanied (`run.sh`) with these images to actually
...
To match conditions in dusts CI, also set the environment variable `DEPLOY=1`, e.g.:
```
...
2. If your Dust source is placed outside of `C:\Users\**`, e.g. if you place the
    repository in the `E:\dust` folder, please add a shared folder from
    VirtualBox by:
...
3. VirtualBox might not support creating symbolic links inside a shared folder
    ...
    VBoxManage setextradata default VBoxInternal2/SharedFoldersEnableSymlinksCreate/e/dust 1
    ::                                                                              ^~~~~~
    ::                                                                              folder name
    ```
