...
int test_dust(size_t (*fn)(va_list), ...) {
    size_t ret = 0;
    va_list ap;
    va_start(ap, fn);
    ret = fn(ap);
...
int main(int argc, char* argv[]) {
    assert(test_dust(check_list_0, 0x01LL, 0x02, 0x03LL) == 0);

    assert(test_dust(check_list_1, -1, 'A', '4', ';', 0x32, 0x10000001, "Valid!") == 0);

    assert(test_dust(check_list_2, 3.14, 12l, 'a', 6.28, "Hello", 42, "World") == 0);

    assert(test_dust(check_list_copy_0, 6.28, 16, 'A', "Skip Me!", "Correct") == 0);
